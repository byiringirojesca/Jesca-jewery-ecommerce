...
@section('content')
    <!-- Outer wrapper uses single quotes to prevent quote collisions with parsed JSON arrays -->
    <div x-data='{ 
                    images: @json(old("images", $product->images ?? ["https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?auto=format&fit=crop&w=800&q=80"])), 
                    addImageSlot() { this.images.push("") },
                    removeImageSlot(index) { this.images.splice(index, 1) }
                 }'
        class="font-sans bg-[#FAFAFA] max-w-auto mx-auto space-y-12 p-2 sm:p-6 text-neutral-800 tracking-normal mix-blend-normal">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between border-b border-neutral-200 pb-8 gap-6">
            <div class="max-w-2xl">
                ...
                <h1 class="font-serif text-4xl sm:text-5xl font-light tracking-wide text-neutral-900">Edit Asset Configuration</h1>
                <p class="font-serif italic text-base text-neutral-500 mt-2 font-light leading-relaxed">
                    Modify active database constraints and runtime instances. Ensure structural integrity remains aligned prior to pipeline execution.
                </p>
            </div>
        ...
        @if ($errors->any())
            <div class="border-l border-red-500 bg-red-50/50 p-4 text-[11px] tracking-widest uppercase font-medium text-red-800">
                <span class="block font-semibold mb-1">Registry Mutation Failure // Review Highlighted Violations Below</span>
            </div>
        @endif

        <!-- Main Mutation Form Card -->
        <div class="border border-neutral-200 bg-white p-6 md:p-10 relative overflow-hidden shadow-[0_4px_25px_-12px_rgba(0,0,0,0.05)]">
            ...
            <form action="{{ route('admin.products.update', $product) }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative shadow-none">
                            ...
                            <input type="number" name="price" step="0.01" min="0.00" value="{{ old('price', $product->price) }}" required
                ...
                <!-- 4. Textarea Information Block Mapping -->
                <div class="space-y-2">
                    <label class="text-[11px] font-semibold uppercase tracking-[0.2em] text-neutral-500 block">
                        Editorial Description & Specifications *
                    </label>
                    <textarea name="description" rows="5" required
                        class="rounded-none border bg-white px-4 py-3 text-sm text-neutral-800 placeholder-neutral-400 focus:outline-none focus:border-[#D4AF37] transition-all duration-300 w-full resize-none @error('description') border-red-400 focus:border-red-400 @else border-neutral-300 @enderror">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <span class="text-[10px] font-medium text-red-600 uppercase tracking-wider block mt-1">// {{ $message }}</span>
                    @enderror
                </div>

                <!-- 5. Dynamic Multi-Image Collection Pipeline Section -->
